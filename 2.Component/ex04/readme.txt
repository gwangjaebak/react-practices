...
01. 기본 개념
  1. state
    ...
    - 상태 업데이트가 되면 컴포넌트의 반응형(Reactive) Rendering이 트리거되고 컴포넌트와 자식 컴포넌트가 다시 렌더링된다.
    - 컴포넌트의 동작(event)와 상호작용을 수행할 수 있는 매커니즘을 제공한다.
...
02. 제어 컴포넌트
  ...
  3. 폼 컴포넌트를 반드시 제어 컴포넌트로 작성해야하는 것은 아니다. 상태를 제어하지 않는
     비제어(Uncontrolled) 컴포넌트로 만들 수 있다.
  4. 예제
    - src/02 제어 컴포넌트    -> 사용자 입력을 상태를 둬서 제한
    - src/03 비제어 컴포넌트  -> 사용자 입력을 상태를 둬서 제한하지 않음
...
03. 상태(Stateful) 컴포넌트 vs 순수(Pure, Dumb) 컴포넌트
  ...
  2. 순수 컴포넌트
    - 상태 관리 없이 속성(props)로 화면만 렌더링하는 컴포넌트
    - 재사용성이 좋다. 테스트하기도 좋다.
  ...
  4. 어떤 컴포넌트가 상태 컴포넌트인가?
    ...
    - 컴포넌트 hierachy에서 상위에 있는 상태를 가져야만하는 컴포넌트
